...
class TalkingHeadAnimefaceInterface():
    ...
    def mp_pose2image_frame(self,global_out_image,current_pose,img_number,user_id,mode,scale,fps):
        ...
        up_scale_image,result = self.mp_get_upscale(global_out_image,out_image,mode,scale,fps)#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
        try:
    ...
    def mp_dic2image_frame(self,global_out_image,current_pose_dic,img_number,user_id,mode,scale,fps,up=True):#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
        frame_start=time.time()
    ...
    def mp_get_upscale(self,global_out_image,out_image,mode,scale,fps): #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
        #result=True   #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
        ...
        if self.queue_out_image.empty()==False:
            ...
            print("++@@@+++ Upscale image is generated")
            result=True  #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
        else:
            result=False   #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
            print("--@@@-- Upscale image is not generated")
        return global_out_image,result

 #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
    def mp_get_inference_queu(self):
        if self.queue_tkh_image.empty()==False:
            out_image = self.queue_tkh_image.get() # queue_in_imageからデータを取得
            result=True
        else:
            out_image=""
            result=False
 ...
 #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<920
    def mp_get_upscale_queu(self):
        if self.queue_out_image.empty()==False:
            out_image = self.queue_out_image.get() # queue_in_imageからデータを取得
            result=True
        else:
            out_image=""
            result=False
